...
const layerHtml = () => `<div class="ms-layer hidden" id="msLayer" aria-label="Milkshake spill">
...
function mountMilkshake(opts) {
  ...
  const setPos = (el, px2, py2) => {
    el.style.left = Math.round(px2) + "px";
    el.style.top = Math.round(py2) + "px";
  };
  ...
  const drops = [];
  let open = false, x = 0, y = 0, rot = 0, level = 1, pouring = false;
  ...
  function lipPoint(px2, py2, r) {
    ...
    return {
      x: px2 + (CUP_VB.pivotX + dx * Math.cos(a) - dy * Math.sin(a)) * s,
      y: py2 + (CUP_VB.pivotY + dx * Math.sin(a) + dy * Math.cos(a)) * s
    };
  }
  function emitFrom(px2, py2, r, lv, color, owner, dt, vx0 = 0) {
    const over = Math.abs(r) - spillAngle(lv);
    if (lv <= 0 || over <= 0) return 0;
    const lip = lipPoint(px2, py2, r);
  ...
  function frame(now) {
    ...
    if (open) {
      ...
      if (made) {
        level = Math.max(0, level - made * 22e-4);
        paintMyLevel();
        report();
      }
    }
  ...
  function tweenRot(to, dur, done) {
    ...
    const from = rot, t0 = (win.performance || performance).now();
    const step = (now) => {
      const p = Math.min(1, (now - t0) / (dur * 1e3));
  ...
  let dragging = false, moved = false, ox = 0, oy = 0, px = 0, py = 0, dragVX = 0, lastDragT = 0;
  ...
  cupEl.addEventListener("pointermove", (e) => {
    ...
    dragVX = dx / Math.max(16, now - lastDragT) * 1e3;
    px = e.clientX;
    py = e.clientY;
    lastDragT = now;
  ...
  layer.addEventListener("click", (e) => {
    ...
    else if (t?.closest('[data-act="ms-wipe"]')) ground.wipe(myUserId());
  ...
  function start() {
    ...
    cupEl.innerHTML = cupSvg(getMyColor(), "me") + (myName ? `<span class="ms-cup-tag" style="color:${safeColor(getMyColor())}">${esc(myName)}</span>` : "");
