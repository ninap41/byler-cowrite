...
export function frameD(w, h, r = RADIUS, i = INSET) {
	const x = w - i, y = h - i
	return (
...
let seq = 0
export function vineSvgHtml(key = ++seq) {
	return (
...
export function mountTendrilBorder(el, { horn = null, gsap = globalThis.window?.gsap, win = globalThis.window } = {}) {
	...
	const kill = () => { tl?.kill(); loop?.kill(); tl = loop = null }
	function settle() { // resting look: the plain accent frame
		kill()
		for (const n of [line, glow, shine]) { n.style.strokeDasharray = "none"; n.style.strokeDashoffset = 0 }
	...
	function render() { paintTheme(); build(); grow() }

	let redraw
	const ro = win?.ResizeObserver ? new win.ResizeObserver(() => {
		clearTimeout(redraw)
		redraw = setTimeout(() => { build(); if (!tl?.isActive()) { settle(); glimmer() } }, 120)
	}) : null
	ro?.observe(el)
	...
	const onVisible = () => {
		...
		if (!loop) { settle(); glimmer() }
	}
	el.ownerDocument.addEventListener("visibilitychange", onVisible)
	render()
	return {
		svg, render, settle, glimmer,
		destroy() { kill(); ro?.disconnect(); mo?.disconnect(); el.ownerDocument.removeEventListener("visibilitychange", onVisible); svg.remove(); el.classList.remove("tendril") },
	}
}
