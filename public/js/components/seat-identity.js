...
//   {kind: "seated", myId, hostId}
export function reconnectOutcome(res, socketId, prevHostId) {
	if (!res?.ok) return { kind: "gone" }
	if (res.pending) return { kind: "pending" }
	return { kind: "seated", myId: socketId, hostId: res.hostId ?? prevHostId }
}
...
export function adoptSeatId(writers, myId, userId) {
	if (!Array.isArray(writers) || userId == null) return null
	if (writers.some((w) => w.id === myId)) return null
	const mine = writers.find((w) => w.userId === userId && w.connected !== false)
