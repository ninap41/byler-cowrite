  ...
  return `<div class="ts ts-game">
	...
	<div class="ts-game-cols">
		<div class="ts-main">
			...
			<div class="ts-mono ts-muted ts-hint">Ctrl/\u2318+Enter to submit \xB7 auto-submits when the timer ends</div>
			<div class="ts-mono ts-muted ts-hint ts-center">Round 3 of 8 \xB7 24 lines \xB7 1,930 words</div>
		</div>
		<aside class="ts-side">
			...
			<div class="ts-mono ts-muted ts-order"><div>1 \xB7 kip</div><div class="ts-acc2">2 \xB7 rose \u2190</div><div>3 \xB7 charlie</div></div>
		</aside>
	</div>
	<div class="ts-chatdock">\u{1F4AC} Chat <span class="ts-badge">3</span><i class="ts-r ts-muted">\u25BE</i></div>
</div>`;
}
function screenLive() {
  return `<div class="ts ts-livecard">
	...
	<div class="ts-notice">\u{1F514} <span>Time's up: the line was committed as typed, and play advanced to <b>charlie</b>.</span></div>
</div>`;
}
const commentCards = (full) => `
	...
	<div class="ts-cmt">
		...
		<p class="ts-mono ts-muted">${full ? "Suggestion" : "suggestion \xB7 author decides"}</p>
	...
	<div class="ts-cmt is-resolved">
		...
		${full ? "" : '<p class="ts-mono ts-muted ts-hint">resolved \xB7 underline dropped</p>'}
	</div>`;
  ...
  return `<div class="ts ts-write">
	<header class="ts-write-head">
		...
		<div class="ts-write-title">
			...
			<div class="ts-meta ts-mono ts-muted"><span class="ts-good">Saved</span><span>\xB7</span><span>4,182 words</span><span>\xB7</span><span class="ts-chip">\u{1F512} Private</span><span class="ts-chip">\u{1F465} Readers \xB7 2</span><span class="ts-chip">\u23F1 Sprint</span><span class="ts-chip on">\u{1F4AC} Comment</span><span class="ts-avs"><i style="background:${R}"></i><i style="background:${Q}"></i></span></div>
		</div>
		${btn("Save", "primary")}
	</header>
	<div class="ts-toolbar dim">${tb("\u21BA")}${tb("\u21BB")}${tb("p \u25BE")}${tb("B", "b")}${tb("I", "i")}${tb("U", "u")}${tb("\u2212 16 +")}${tb("\u{1F517}")}${tb("\u{1F5BC}")}<span class="ts-r ts-modes"><span class="ts-chip">Rich text</span><span class="ts-muted">HTML</span><span class="ts-muted">Comment</span></span></div>
	<div class="ts-write-cols">
		...
		<aside class="ts-cmts">
			<div class="ts-cmts-head"><h3>\u{1F4AC} Comments</h3><span class="ts-r ts-mono ts-muted">2 open</span></div>
			${commentCards(true)}
		</aside>
	</div>
</div>`;
...
function screenComment() {
  return `<div class="ts ts-comment">
	<div>
		<div class="ts-mono ts-muted ts-modebar"><span class="ts-chip on">\u{1F4AC} Comment mode</span><span>beta reader \xB7 read and comment.</span></div>
...
const rung = (label, words, pct, state) => `<div class="ts-rung ${state}"><span class="ts-rung-name">${label}</span><span class="ts-mono ts-muted">${words}</span><span class="ts-bar"><i style="width:${pct}%"></i></span><span class="ts-mono ts-rung-state">${state === "earned" ? "earned" : state === "current" ? "current" : pct + "%"}</span></div>`;
function screenRanks() {
  return `<div class="ts ts-ranks">
	...
	${rung("\u{1FA84} Practice", "10,000", 100, "earned")}
	${rung("\u{1F9D9} Sorcerer", "20,000", 100, "earned")}
	${rung("\u{1F9ED} Artist", "50,000", 100, "current")}
	${rung("\u{1F624} It's not my fault you don't like girls!", "65,000", 68, "next")}
	${rung("\u{1F300} Crazy Together", "150,000", 35, "next")}
	<h3 class="ts-h ts-rule">Collected badges</h3>
	<div class="ts-badges"><span class="ts-chip">\u{1F52B} There. Out Loud.</span><span class="ts-chip">\u{1F436} Puppy Mike</span><span class="ts-chip">\u{1FAF6} You have explored</span><span class="ts-chip">\u26A1 Innate Powers</span><span class="ts-chip">\u2601\uFE0F I miss clouds I miss you</span><span class="ts-chip">\u{1FA96} Like the soldiers</span><span class="ts-chip secret">\u{1F512} secret badge</span><span class="ts-chip secret">\u{1F512} secret badge</span></div>
	<div class="ts-notice">\u{1F3C6} <span>Unlocked mid-line: <b>charlie</b> earned <b>\u{1FA84} Practice</b>, announced to the lobby.</span></div>
</div>`;
}
function screenGimmick() {
  return `<div class="ts ts-gimmick">
	...
	<div class="ts-notice">\u{1F3B2} <span><b>rose</b> rolled a <b class="ts-acc">natural 20</b>, the turn is stolen.</span></div>
	<div class="ts-badges"><span class="ts-chip">\u{1F3B2} Play gimmick</span><span class="ts-chip">\u{1F525} Hellfire d20</span><span class="ts-chip locked">\u{1F964} Milkshake \xB7 locked</span><span class="ts-chip locked">\u{1FAA9} Disco ball \xB7 locked</span></div>
</div>`;
}
function screenDash() {
  return `<div class="ts ts-dash">
	<div class="ts-dash-main">
		<div class="ts-me">
			...
			<div>
				<div class="ts-me-row"><b style="color:${K}">kip</b><span class="ts-chip">\u{1F9ED} Artist</span><span class="ts-r ts-acts">${btn("View profile", "ghost")}${btn("Log out", "ghost")}</span></div>
		...
		<div class="ts-stats">
			...
			<div class="ts-streak"><svg viewBox="0 0 70 70"><circle cx="35" cy="35" r="29" class="ring-bg"></circle><circle cx="35" cy="35" r="29" class="ring"></circle><text x="35" y="33" class="fire">\u{1F525}</text><text x="35" y="48" class="n">9</text></svg><div class="ts-muted"><b>9 days</b> current streak<br />Best: 21 days</div></div>
		</div>
		<div><span class="ts-muted ts-hint">\u{1F9ED} Artist \xB7 12,860 words to \u{1F624} It's not my fault you don't like girls!</span><div class="ts-bar wide"><i style="width:68%"></i></div></div>
		<div class="ts-rule">
			<h3 class="ts-h">Games in progress</h3>
			<div class="ts-games">
				<div class="ts-gcard"><div class="ts-cover grad"><span class="ts-ago">2m ago</span><span class="ts-cover-code">M6</span></div><div class="ts-gbody"><div class="ts-gtitle"><span>Supersucker</span><span class="ts-mono ts-muted">M6DH</span></div><p class="ts-good">Your turn \xB7 0:14 left</p><div class="ts-dots"><i style="background:${K}"></i><i style="background:${R}"></i><i style="background:${Q};opacity:.35"></i><span class="ts-muted">3 writers</span></div></div>${btn("Resume: your turn", "primary")}</div>
				<div class="ts-gcard"><div class="ts-cover alt"><span class="ts-cover-code">ZD</span></div><div class="ts-gbody"><div class="ts-gtitle"><span>Rain Check</span><span class="ts-mono ts-muted">ZDU4</span></div><p class="ts-acc">Paused by host \xB7 asleep</p><div class="ts-dots"><i style="background:${R};opacity:.35"></i><i style="background:#37e0a0;opacity:.35"></i><span class="ts-muted">2 writers</span></div></div>${btn("Wake it up", "ghost")}</div>
				<div class="ts-gcard empty"><span>\u2728</span><span>Open slot: start a new story or invite friends.</span></div>
			</div>
		</div>
	</div>
	<div class="ts-dash-rail">
		<nav class="ts-nav"><div class="on"><span>\u{1F4EC}</span><span>Inbox</span><span class="ts-badge">2</span></div><div><span>\u2728</span><span>Start a game</span><span class="ts-muted">\u2192</span></div><div><span>\u{1F511}</span><span>Join a game</span><span class="ts-muted">\u25BE</span></div><div><span>\u2712\uFE0F</span><span>New solo write</span></div><div><span>\u{1F4DA}</span><span>My solo writes</span></div></nav>
		<div>
...
function screenThemes(total = 19) {
...
const TOUR = [
  ...
  { id: "badges", kicker: "Badges", title: "A word-count ladder, and the secret ones.", body: "From \u{1F436} Puppy Mike (5,000 words) to \u{1F300} Crazy Together (150,000), plus secret badges earned by writing\u2026 the right thing (some need the right <i>combination</i> of words in one line). Unlocks announce to the whole session.", screen: screenRanks },
  { id: "gimmicks", kicker: "Gimmicks", title: "Rank rewards you play to distract the table.", body: `"\u{1F3B2} Play gimmick" hands you a d20 in your own colour to throw around the screen, over the story, over the editor, over everyone's screen, the writer's included. Every landing is called in the chat, and a natural 20 steals the turn.`, screen: screenGimmick },
