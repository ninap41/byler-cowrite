...
const caret = () => `<span class="ts-caret"></span>`
const btn = (label, cls = "") => `<span class="ts-btn ${cls}">${label}</span>`
const tb = (label, cls = "") => `<span class="ts-tb ${cls}">${label}</span>`
...
const rung = (label, words, pct, state) =>
...
export function screenThemes(total = 19) {
