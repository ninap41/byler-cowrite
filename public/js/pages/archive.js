...
mountViewPicker(document.getElementById("archViewWrap"), (cols) => {
  ...
  $("archiveList").querySelectorAll(".arch-group").forEach((box) => applyGrid(box, cols));
});
function renderGameCards(games) {
  const list = $("archiveList");
  if (!games.length) {
    list.innerHTML = '<p class="archive-empty">No saved games yet, finish (or pause) a game and it will show up here.</p>';
    return;
  }
  list.innerHTML = "";
  const groups = [
...
async function openGameDetail(code) {
  ...
  try {
    ...
    if (e.status === 403) location.replace("/stories?code=" + encodeURIComponent(code));
    return;
  }
  curG = g;
  ...
  $("archMeta").textContent = `code: ${g.code} · ${g.writers.length} writer${g.writers.length === 1 ? "" : "s"} · ${Number(g.words || 0).toLocaleString()} words · last played ${fmtWhen(g.savedAt)}`;
  $("archPrompt").innerHTML = promptHtml(g.prompt || "(no prompt)");
  const paintTags = (tags) => {
  ...
  $("archTagEdit").onclick = () => {
    ...
    $("archTags").querySelector("input")?.focus();
  };
...
const loadJsPdf = () => jsPdfPromise ??= new Promise((res, rej) => {
...
function storyToPdf(jspdf, prompt, story, code) {
  ...
  const M = 64, W = doc.internal.pageSize.getWidth() - M * 2, BOTTOM = doc.internal.pageSize.getHeight() - M;
  let y = M;
...
$("delConfirm").onclick = async () => {
  try {
    ...
    setTimeout(() => note.classList.add("hidden"), 6e3);
