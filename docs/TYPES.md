...
| a solo write | `Doc { id; ownerId; title; chapters: Chapter[]; betaReaders: UserId[]; visibility: Visibility; rev; sprints; sprintWords; createdAt; updatedAt }` plus `CommentRecord { docId; comments: DocComment[] }` (JSDoc typedefs at the top of `src/docs.js`; `DocCommentPos` in `wire.ts`) | `html` and `comments` are ATTACHED on read and never persisted in the doc blob; `rev` is the save counter a quiet save conflicts on; `wordCount` is derived — the type says so by leaving them off `Doc` and putting them on `DocPayload` |
| a kept copy of a solo write | cold blob `dochist/<docId>_<at>_<reason>_<words>_<chapters>` = `{ title; words; chapters: Pick<Chapter, "id" \| "title" \| "html">[] }` (`snapshotOf` in `src/dochist.js`); the listing is parsed from the NAME, `HistoryMeta { at; reason: "time" \| "drop" \| "restore"; words; chapters }` (`lib/doc-history.js`) | `VersionRow` (write-view.ts) is the wire row of `GET /api/docs/:id/history`; `storage.cold` kinds are async and never in memory |
