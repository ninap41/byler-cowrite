...
| `achievements.json` | `wordTiers` — the rank ladder (id, name, min words, emoji); `usageOpen` — word badges whose descriptions are always visible; `usage` — the SECRET word badges (trigger words and descriptions hidden until earned); a usage badge may add `sound`, the name of an mp3 in `sounds/` (no extension) played once for the whole table when it lands; `themeUnlocks` — which rank earns which theme (and, through the theme, which gimmick). See `docs/UNLOCKS.md`. |
