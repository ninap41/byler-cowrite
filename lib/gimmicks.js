...
export const GIMMICKS = {
  ...
  galaga: {
    ...
    theme: "arcade",
    desc: "Blast the Galaga fleet over the live game. Your run lands in the chat — and beating 3,000 steals the turn.",
  },
  milkshake: {
    id: "milkshake",
...
// call in game.js.
export const GALAGA_TARGET = 3000;
...
// what actually happened, `declined` means the player had opted out.
export function describeGalaga(outcome, { stole = false, from = "", declined = false } = {}) {
  const pts = outcome.score.toLocaleString();
  if (outcome.kind === "highscore") return stole
    ? `blasted the fleet for ${pts} 👾 — beat ${GALAGA_TARGET} and stole the turn${from ? ` from ${from}` : ""}!`
    : declined
      ? `blasted the fleet for ${pts} 👾 — beat ${GALAGA_TARGET}, and let the writer keep the turn.`
      : `blasted the fleet for ${pts} 👾 — beat ${GALAGA_TARGET}!`;
