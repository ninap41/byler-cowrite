...
// achievements.json's gimmickUnlocks maps a gimmick id to the tier that earns
// it; the registry of what a gimmick IS is GIMMICKS in lib/gimmicks.js once it
// exists — until then the pool is simply empty and nothing here minds.
export const GIMMICK_UNLOCKS = Object.fromEntries(
  Object.entries(CFG.gimmickUnlocks || {}).filter(([, tierId]) => TIER_IDS.includes(tierId)),
);
const GIMMICK_LABELS = CFG.gimmickLabels || {};
