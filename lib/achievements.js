...
import { THEME_LABELS } from "../public/js/shared/themes.js";
...
// A usage badge may carry a sound clip: the basename of a file in sounds/
// (no extension), played once for the whole table when the badge lands.
export const badgeSound = (id) => ALL.find((b) => b.id === id)?.sound ?? null;
export const SOUND_RE = /^[a-z0-9]+(?:[-_][a-z0-9]+)*$/;
...
export function rewardsForTiers(tierIds) {
  /** @type {{themes: {id: string, name: string}[], gimmicks: {id: string, name: string}[]}} */
...
export function validateAchievements(next) {
  ...
  for (const pool of ["usage", "usageOpen"]) {
    ...
    for (const b of list) {
      ...
      if (b?.sound != null && b.sound !== "" && !(typeof b.sound === "string" && SOUND_RE.test(b.sound.trim())))
        errors.push(`${pool}: "${b?.id}" sound must be a file name like vecna-laugh (a file in sounds/, no extension).`);
    }
  }
...
const cleanUsage = (b) => ({
  ...
  ...(isStr(b.sound) ? { sound: b.sound.trim() } : {}),
