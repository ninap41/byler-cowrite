...
// Features (not themes, not gimmicks) that a rank hands out: the registry of
// what can be gated and the label an announcement / the ranks page uses.
export const FEATURE_LABELS = { reference: "Writers' reference palette" };
export let FEATURE_UNLOCKS = {};
...
// ---- Features as rank rewards (a third pool, the theme shape again) ------
// featureUnlocks in achievements.json maps a FEATURE_LABELS id to a tier.
// Unlike themes this one is enforced: /api/reference refuses a locked caller.
export const tierForFeature = (id) => FEATURE_UNLOCKS[id] ?? null;
export const featureLocks = () =>
  Object.fromEntries(
    Object.entries(FEATURE_UNLOCKS).map(([id, tierId]) => {
      const t = WORD_TIERS.find((x) => x.id === tierId);
      return [id, { tier: tierId, name: t.name, min: t.min }];
    }),
  );
export function canUseFeature(u, id) {
  const tierId = tierForFeature(id);
  if (!tierId) return true;
  if (u?.admin === true) return true;
  return (u?.badges || []).includes(tierId);
}
// Every feature in the registry this account may use (free ones included).
export const unlockedFeatures = (u) => Object.keys(FEATURE_LABELS).filter((id) => canUseFeature(u, id));
...
export function rewardsForTier(tierId) {
  ...
  return { themes: pick(THEME_UNLOCKS, THEME_LABELS), gimmicks: pick(GIMMICK_UNLOCKS, GIMMICK_LABELS), features: pick(FEATURE_UNLOCKS, FEATURE_LABELS) };
}
...
export function describeRewards(r) {
  ...
  if (r.features?.length) parts.push(`the ${list(r.features)}`);
...
export function applyCatalogue(cfg) {
  ...
  FEATURE_UNLOCKS = Object.fromEntries(
    Object.entries(CFG.featureUnlocks || {}).filter(([id, tierId]) => id in FEATURE_LABELS && TIER_IDS.includes(tierId)),
  );
...
export function validateAchievements(next) {
  ...
  if (next.featureUnlocks && typeof next.featureUnlocks !== "object") errors.push("featureUnlocks: must be an object of feature → rank id.");
  return errors;
}
...
export function setAchievements(next) {
  ...
  const doc = {
    ...
    featureUnlocks: next.featureUnlocks && typeof next.featureUnlocks === "object" ? next.featureUnlocks : CFG.featureUnlocks || {},
  };
