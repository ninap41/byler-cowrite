...
export function generateIntermediatePrompt(data, options = {}) {
  ...
  if (explicitLevel === "explicit" && isAdult(season)) {
    ...
    const setup = pinned(ex.setups, "setupId") || one(ex.setups);
    addTags(ctx.activeTags, setup);
    const dynamic = pinned(ex.dynamics, "dynamicId") || one(ex.dynamics);
    ...
    const acts = some(ex.acts, "actId");
    for (const a of acts) addTags(ctx.activeTags, a);
    const kinks = some(ex.kinks, "kinkId");
    const register = pinned(ex.registers, "registerId") || one(ex.registers);
