...
// `suggestive` is deprecated: off the wire it reads as none, and an old pack's
// level row is ignored.
export const EXPLICIT_LEVELS = ["none", "explicit"];
// How many kinks a Kinks line may carry; the host picks 1..MAX_KINKS, default 1.
export const MAX_KINKS = 3;
...
// Age comes from the SEASON and nowhere else: future fic (id post-canon) is the
// adult one, every on-screen season is minors.
export const isAdult = (season) => season?.ageGroup === "adult";
// Where the explicit layer may go: the adult season, or a season the pack
// marks `explicit-ok` (S4 and S5). Everything earlier also carries
// `incompatibleTags: ["explicit"]`, so the menu greys it once Explicit is
// chosen and the generator's season draw narrows the same way.
export const allowsExplicit = (season) => !!season && (isAdult(season) || (season.tags || []).includes("explicit-ok"));
...
export function isCompatible(item, context = {}) {
  ...
  if (item.compatibleAgeGroups?.length && season && !item.compatibleAgeGroups.includes(season.ageGroup))
    return false;
  // `adultOnly` marks explicit-only content (the explicit level, a world's
  // explicit rooms): it never reaches a season the explicit layer can't.
  if (item.adultOnly && season && !allowsExplicit(season)) return false;
...
/** @param {string[]} prompts @param {{ recent?: string[], rng?: () => number }} [opts] */
...
// The explicit gate. It runs BEFORE any tag weighting: a season the layer
// can't reach (pre-canon through S3) forces the level to none and hides the
// kink layer entirely; it is not a filter applied to a dealt result.
export function gateExplicit(level, season) {
  const want = EXPLICIT_LEVELS.includes(level) ? level : "none";
  if (want === "explicit" && !allowsExplicit(season)) return "none";
  return want;
}
...
export function generateIntermediatePrompt(data, options = {}) {
  ...
  const seasonPool = !seasonPicked && askedLevel === "explicit" && (data.seasons || []).some(allowsExplicit)
    ? data.seasons.filter(allowsExplicit) : data.seasons;
  ...
  // The situation is drawn BEFORE the relationship because one situation
  // rules the whole relationship axis out: a first meeting (tag
  // `first-meeting`, which every relationship refuses) has no relationship
  // yet, so the line is not dealt and a host's pick is ignored. situationOff
  // leaves the situation out the way toneOff does.
  const situation = options.situationOff ? null : resolve(data.situations, want("situationId"), ctx, rng, recent);
  addTags(ctx.activeTags, situation);
  const noRelationship = !!situation?.tags?.includes("first-meeting");
  // A relationship the host pinned that the situation rules out (a reunion
  // with an established couple) is dealt around, not honoured: the menu
  // greys it, and the generator agrees even when the pick comes off the wire.
  const relWanted = byId(data.relationships, want("relationshipId"));
  const relOk = !relWanted || isCompatible(relWanted, ctx);
  const relationship = noRelationship ? null : resolve(data.relationships, relOk ? want("relationshipId") : "random", ctx, rng, recent);
  addTags(ctx.activeTags, relationship);
  // The host can leave the tone out altogether (toneOff): no line, no tags.
  const tone = options.toneOff ? null : resolve(data.tones, want("toneId"), ctx, rng, recent);
  addTags(ctx.activeTags, tone);
  // …and vice versa: a tone the host CHOSE that can't be explicit brings the
  // rating down to none rather than dealing explicit fluff.
  ...
  // A world the host CHOSE that can't be explicit (high school — kids) steps
  // the rating down the way a soft tone does, instead of dealing it anyway.
  if (explicitLevel === "explicit" && world?.incompatibleTags?.includes("explicit")) {
    explicitLevel = "none";
    ctx.activeTags.delete("explicit");
  }
  ...
  // places — so under a world the Place axis deals from `auPlaces` instead:
  // each world's own rooms, keyed by the world's `au-<id>` tag (an explicit
  // one also requires the `explicit` tag, so it never reaches a softer
  // ballot or a minor season). The generic pool is canon-only.
  // A pinned placeId is honoured from whichever pool is in play: one of the
  // world's rooms under a world, a generic place otherwise; a pin from the
  // other pool (or one the gate refuses) is simply dealt around.
  const auPool = world ? (data.auPlaces || []).filter((p) => isCompatible(p, ctx)) : [];
  const pinnedRoom = world ? byId(auPool, want("placeId") !== "random" ? want("placeId") : null) : null;
  const place = world
    ? pinnedRoom || (auPool.length ? drawDistinct(auPool, 1, ctx, rng, recent)[0] || null : null)
    : resolve(data.places, want("placeId"), ctx, rng, recent);
  ...
  /** @type {any} */
  let explicit = null;
  if (explicitLevel === "explicit") {
    const one = (list) => drawDistinct(list, 1, ctx, rng, recent)[0] || null;
    // The host may pin any part of the line (setupId / dynamicId / actId /
    // kinkId / registerId, the explicit dropdowns); a pinned act or kink
    // leads its list and the rest is dealt around it.
    const pinned = (list, key) => byId(list, want(key) && want(key) !== "random" ? want(key) : null);
    // acts: one or two; kinks: exactly kinkCount (default ONE — the host
    // asks for more, they are never piled on), the pinned one leading
    /** @param {any[]} list @param {string} key @param {number | null} [count] */
    const some = (list, key, count = null) => {
      const first = pinned(list, key);
      const n = count ?? 1 + Math.floor(rng() * 2);
      if (!first) return drawDistinct(list, n, ctx, rng, recent);
      return [first, ...drawDistinct(list, n - 1, ctx, rng, recent, new Set([first.id]))];
    };
    const kinkCount = Math.min(MAX_KINKS, Math.max(1, Number.isInteger(options.kinkCount) ? options.kinkCount : 1));
    // …and may switch any part off (setupOff / dynamicOff / actOff /
    // kinkOff): a part that's off is never drawn, so it never reaches the
    // card. (Registers are no longer dealt at all.)
    const setup = options.setupOff ? null : pinned(ex.setups, "setupId") || one(ex.setups);
    addTags(ctx.activeTags, setup);
    const dynamic = options.dynamicOff ? null : pinned(ex.dynamics, "dynamicId") || one(ex.dynamics);
    ...
    const acts = options.actOff ? [] : some(ex.acts, "actId");
    for (const a of acts) addTags(ctx.activeTags, a);
    const kinks = options.kinkOff ? [] : some(ex.kinks, "kinkId", kinkCount);
    ...
    explicit = { setup, dynamic, acts, kinks, twist, who };
  }
  ...
  if (explicit) {
    ...
    // the whole layer is one line — setup, dynamic, acts, kinks — read
    // together as the tags on the work; with every part switched off the
    // Rating line stands alone
    parts.push(line("Kinks", [explicit.setup, explicit.dynamic, ...explicit.acts, ...explicit.kinks, explicit.twist]
  ...
  return {
    ...
    selections: {
      ...
      ...(relationship ? { relationshipId: relationship.id } : {}),
      ...
      ...(explicit ? {
        explicit: {
          ...
          actIds: idsOf(explicit.acts), kinkIds: idsOf(explicit.kinks),
    ...
    labels: {
      ...
      ...(situation ? { situation: situation.label } : {}),
...
// `registers` is deprecated: still accepted in a pack, validated when present, never dealt.
export const EXPLICIT_POOLS = ["levels", "setups", "dynamics", "acts", "kinks"];
...
export function validateIntermediateData(data) {
  ...
  if (data.auPlaces) validatePool(errors, "auPlaces", data.auPlaces); // optional: a world's own rooms
  for (const s of data.seasons || [])
    if (s.ageGroup !== "minor" && s.ageGroup !== "adult") errors.push(`seasons/${s.id}: ageGroup must be minor or adult`);
  if (!(data.seasons || []).some(allowsExplicit)) errors.push("seasons: no season allows explicit, the layer is unreachable");
  ...
  if (ex.registers) validatePool(errors, "explicit/registers", ex.registers); // deprecated, optional
