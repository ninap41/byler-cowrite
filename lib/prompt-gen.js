...
// role ("{name} is the brat"). A tag with no role, or no character, is bare.
export function withWho(tag, who) {
  if (!tag?.who || !who) return tag?.text || "";
  return `${tag.text} (${String(tag.who).replace(/\{name\}/g, who)})`;
}
...
export function generateIntermediatePrompt(data, options = {}) {
  ...
  const explicitLevel = gateExplicit(askedLevel, season);

  // Canon second — it decides whether a setting AU is thinkable at all.
  const canon = resolve(data.canon, want("canonId"), ctx, rng, recent);
  ctx.canon = canon;
  ...
  const world = canon?.id === "au"
    ? (byId(data.tropes, locked.worldId)?.group === "setting-au" ? byId(data.tropes, locked.worldId) : null)
      || drawDistinct(data.tropes, 1, { ...ctx, group: "setting-au" }, rng, recent)[0] || null
    : null;
  ...
  if (explicitLevel === "explicit" && isAdult(season)) {
    ...
    explicit = { setup, dynamic, acts, kinks, register, who };
  }
  ...
  if (explicit) {
    ...
    parts.push(line("Kinks", [explicit.setup, explicit.dynamic, ...explicit.acts, ...explicit.kinks, explicit.register]
