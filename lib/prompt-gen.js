...
export function validateIntermediateData(data) {
  ...
  for (const t of data.tensions || []) {
    ...
    // The Scenario type menu shows categoryLabels, so every category a tension
    // uses must be named there — an unlabelled one would surface as a bare id.
    if (t.category && !data.categoryLabels?.[t.category]?.trim())
      errors.push(`tensions/${t.id}: category ${t.category} has no categoryLabels entry`);
  }
