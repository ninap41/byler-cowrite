...
test("menuHtml: friendly game / unseated / locked / a tablemate's unlock / play", () => {
  ...
  assert.match(table, /class="gd-menu-item table"[^>]*data-act="play"[^>]*>🔓 Hellfire d20 · Play/, "a tablemate's rank hands everyone at the table the toy");
  assert.ok(!/gd-menu-item table"[^>]*disabled/.test(table), "not disabled, not aria-disabled — the row is live");
  assert.match(table, /A tablemate has this unlocked, so it&#39;s yours at this table · Unlocks at 🧙 Sorcerer · 20,000 words to keep it/);
  assert.match(menuHtml({ ...gate, friendly: false, unlocked: ["d20"], table: ["d20"] }), /class="gd-menu-item" data-gimmick="d20" data-act="play">🎲 Hellfire d20 · Play/, "my own rank wins, no open lock");
