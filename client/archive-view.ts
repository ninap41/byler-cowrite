...
import type { Who, Phase } from "./shared/wire.js"
...
/** A game as GET /api/games lists it (gameSummary() in src/game.js). */
export interface ArchiveGame extends GameRef {
	name?: string
	prompt?: string
	phase: Phase | string
	savedAt?: number
	...
	words?: number
	writers: { name: string; color?: string; isHost?: boolean }[]
}
/** A public solo write on the all-stories shelf (a `kind: "write"` row of /api/stories). */
...
export const canContinue = (g: Pick<ArchiveGame, "hostName"> | null | undefined, username: string | null | undefined): boolean =>
	!!g && !!username && !!g.hostName && g.hostName === username

export function gameCardHtml(g: ArchiveGame): string {
...
export function writeCardHtml(d: WriteCard): string {
	return (
...
export const STORY_VIEWS: readonly StoryView[] = [
...
export const storyView = (key: unknown): StoryView => STORY_VIEWS.find((v) => v.key === key) || STORY_VIEWS[0]!
export const cleanStoryView = (key: unknown): string => storyView(key).key

// A segmented control: the pressed segment IS the current layout.
export const viewToggleHtml = (current: unknown): string =>
...
export const archiveMetaText = (g: Pick<ArchiveGame, "code" | "phase" | "savedAt">): string =>
	`${g.code} · ${g.phase === "over" ? "finished" : "paused"} · ${fmtWhen(g.savedAt)}`

// Story html in snapshots already passed through sanitizeRich() server-side.
export function archiveStoryHtml(story: (Who & { html?: string })[]): string {
	if (!story.length) return '<p class="empty">Nothing written yet.</p>'
	return story
		.map((l) => `<div class="story-line"><span class="line-by"><span class="who" style="color:${safeColor(l.color)}">${esc(l.name)}</span>${whoMarks(l)}</span>${l.html || ""}</div>`)
		.join("")
}
