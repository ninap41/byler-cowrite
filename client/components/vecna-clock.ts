...
function caseHtml(c: ClockSpec): string {
	const box = (y0: number, y1: number, half: number, cls: string): string =>
		`<rect x="${r1(c.cx - half)}" y="${y0}" width="${r1(half * 2)}" height="${r1(y1 - y0)}" class="${cls}" />`
	const moulding = (y: number, half: number, h = 14): string =>
...
function glassHtml(c: ClockSpec): string {
...
function dialHtml(c: ClockSpec): string {
	...
	const hand = (angle: number, len: number, cls: string): string => {
...
export function clockSvg(c: ClockSpec = CLOCK): string {
