...
export interface GimmickGate {
	...
	/** ids ANY seated account has unlocked (the server's tableHasGimmick) */
	table?: readonly string[]
}
export interface DiceMenuGate extends GimmickGate {
	friendly?: boolean
	seated?: boolean
}
...
/** A tablemate's die on the wire: where it sits as fractions of their screen. */
...
const lockTip = (lock: Lock): string => typeof lock === "string" ? `Unlocks at ${lock}` : `Unlocks at ${lock.name}${lock.min ? " · " + lock.min.toLocaleString() + " words" : ""}`
...
export function menuHtml({ catalogue = [], unlocked = [], admin = false, locks = {}, friendly = true, seated = true, table = [] }: DiceMenuGate): string {
...
export const hintHtml = (): string => `<span class="gd-hint">Drag to move · flick or click to roll · a natural 20 steals the turn</span>`
export function resultHtml({ value, kind, stole, declined = false }: RollResult): string {
	if (kind === "crit")
		return (
...
export interface DiceOpts extends GimmickMountOpts {
	isSeated?(): boolean
	isFriendly?(): boolean
	onEnter?(): void
	onExit?(): void
	/** any die starts tumbling — mine or a tablemate's; the roll sound */
	onRoll?(r: RollResult | { userId: string; value: number }): void
	onLand?(r: RollResult): void
	/** a menu pick that is NOT a die (the Galaga game…) is handed to its own mount */
	launchers?: Partial<Record<string, () => void>>
}
export interface DiceMenu extends Omit<Gimmick, "start"> {
	setGate(g: Partial<GimmickGate>): void
...
export function mountGimmickDice(opts: DiceOpts): DiceMenu {
	...
	let gate: Required<GimmickGate> = { catalogue: [], unlocked: [], admin: false, locks: {}, table: [] }
	let open = false // MY die is out
	let current: string | null = null // gimmick id in play
	let die: Die | null = null
	const remote = new Map<string, Remote>() // everyone else's dice
	// opt out of stealing: unchecked = a natural 20 is still called, the turn stays
	const stealBox = $("gdSteal") as HTMLInputElement
	...
	menu.addEventListener("click", (e) => {
		e.stopPropagation()
		const b = (e.target as Element | null)?.closest<HTMLElement>("button[data-act]")
		if (!b) return
		showMenu(false)
	...
	const bounds = (s: number) => ({ minX: 4, minY: 4, maxX: vw() - s - 4, maxY: vh() - s - 4 })
	const toFrac = (px: number, py: number, s: number) => {
		const b = bounds(s)
		return { x: b.maxX > b.minX ? (px - b.minX) / (b.maxX - b.minX) : 0, y: b.maxY > b.minY ? (py - b.minY) / (b.maxY - b.minY) : 0 }
	}
	const fromFrac = (fx: number, fy: number, s: number) => {
		const b = bounds(s)
	...
	let x = 0,
		y = 0
	function place(nx: number, ny: number, { send = true }: { send?: boolean } = {}) {
	...
	function enter(id: string = "d20") {
		const launch = opts.launchers?.[id]
		if (launch) return launch() // not a die — its own game takes it from here
		if (open) return
		open = true
		current = id
	...
	function throwTo(tx: number, ty: number) {
		const b = bounds(dieSize())
	...
	function upsertRemote({ userId, name, color, x: fx, y: fy }: RemoteDie) {
	...
	socket.on("gimmick-dice", (list: RemoteDie[]) => {
	...
	socket.on("gimmick-roll", (r: { userId: string; value: number }) => {
	...
	return {
		gimmicksOff,
		setGate(g) {
			gate = { ...gate, ...g } as Required<GimmickGate>
