...
export const TOOLBAR_CONTROLS: readonly string[] = [
	"undo", "redo",
	"blockFormat",
...
export function toolbarHtml(idPrefix = ""): string {
...
export function mountRichToolbar(editor: HTMLElement, toolbar: HTMLElement, { onEdit = () => {}, idPrefix = "" }: RichToolbarOpts = {}): RichToolbar {
	const $ = <T extends HTMLElement = HTMLElement>(n: string): T | null => document.getElementById(idPrefix + n) as T | null
	...
	const press = (el: HTMLElement | null | undefined, fn: () => void) =>
	...
	toolbar.querySelectorAll<HTMLElement>("button[data-cmd]").forEach((b) => press(b, () => exec(b.dataset.cmd ?? "")))
	...
	press($("clearFmtBtn"), () => {
		...
		for (const cmd of ["bold", "italic", "underline", "strikeThrough"])
			if (document.queryCommandState(cmd)) document.execCommand(cmd, false, undefined)
		edited()
		sync()
	})
	...
	$<HTMLSelectElement>("blockFormat")?.addEventListener("change", (e) => {
		editor.focus()
		document.execCommand("formatBlock", false, "<" + (e.target as HTMLSelectElement).value + ">")
		const sel = window.getSelection()
		if (sel?.rangeCount) clearSizesInBlocks(editor, sel.getRangeAt(0))
		edited()
		sync()
	})
	$<HTMLSelectElement>("listSelect")?.addEventListener("change", (e) => {
		const v = (e.target as HTMLSelectElement).value
		editor.focus()
		if (v === "none") {
			for (const c of ["insertUnorderedList", "insertOrderedList"])
				if (document.queryCommandState(c)) document.execCommand(c, false, undefined)
		} else document.execCommand(v, false, undefined)
		edited()
		sync()
	})
	$<HTMLSelectElement>("alignSelect")?.addEventListener("change", (e) => exec((e.target as HTMLSelectElement).value))
	...
	const sizeForStep = () => {
		...
		return parseSize($<HTMLInputElement>("fsInput")?.value) ?? DEFAULT_SIZE
	}
	...
	const holdSel = (on: boolean) => editor.classList.toggle("hold-sel", on)
	$("fontStepper")?.addEventListener("mousedown", () => (remember(), holdSel(true)))
	$<HTMLInputElement>("fsInput")?.addEventListener("focus", () => $<HTMLInputElement>("fsInput")?.select())
	$("fsInput")?.addEventListener("blur", () => holdSel(false))
	editor.addEventListener("mousedown", () => holdSel(false))
	const applyFromBox = () => {
		const px = parseSize($<HTMLInputElement>("fsInput")?.value) // "24px" and "24" both mean 24
		editor.focus()
	...
	function sync() {
		const sizes = sizesNow()
		const box = $<HTMLInputElement>("fsInput")
		...
		try {
			const list = $<HTMLSelectElement>("listSelect")
			if (list)
				list.value = document.queryCommandState("insertUnorderedList")
					...
					: document.queryCommandState("insertOrderedList")
						? "insertOrderedList"
						: "none"
			const align = $<HTMLSelectElement>("alignSelect")
			if (align)
			...
			for (const [cls, cmd] of [["tb-b", "bold"], ["tb-i", "italic"], ["tb-u", "underline"], ["tb-s", "strikeThrough"]] as const)
				toolbar.querySelector("." + cls)?.classList.toggle("on", document.queryCommandState(cmd))
		} catch (e) {}
		syncBlock()
	}
	function syncBlock() {
		const picker = $<HTMLSelectElement>("blockFormat")
		if (!picker) return
	...
	const nativeUndoField = (el: EventTarget | Element | null): boolean => !!el && el !== editor && ((el as Element).tagName === "INPUT" || (el as Element).tagName === "TEXTAREA")
