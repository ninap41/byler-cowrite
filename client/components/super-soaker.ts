...
/** A shot on the wire: where it left from, the aim, and the seed every viewer simulates from. */
...
export interface SoakerGimmick extends Gimmick {
	fire(): void
...
export function gunHtml(key: string, color = "#38bdf8"): string {
	void key // ids are per-gun on the milkshake; the emoji gun needs none, the argument stays for symmetry
	const c = safeColor(color)
	return `<span class="sk-gun3d" style="--sk-c:${c}"><span class="sk-emoji">🔫</span></span>`
}

export const layerHtml = (): string => `<div class="sk-layer hidden" id="skLayer" aria-label="SuperSoaker">
...
export function mountSuperSoaker(opts: GimmickMountOpts): SoakerGimmick {
	const { socket } = opts
	...
	const groundC = doc.getElementById("skGround") as HTMLCanvasElement
	...
	const setPos = (el: HTMLElement, px: number, py: number, angle = 0) => {
		el.style.left = Math.round(px) + "px"
		el.style.top = Math.round(py) + "px"
		const g = el.querySelector<HTMLElement>(".sk-gun3d")
		if (!g) return
	...
	const gunInner = (key: string, name: string | undefined, color: string | undefined): string =>
	...
	function startBurst({ x, y, angle, seed, color, userId: owner }: Squirt) {
	...
	function endBurst(key: string, { fade = false }: { fade?: boolean } = {}) {
	...
	const remote = new Map<string, { el: HTMLElement }>()
	const syncLayer = () => {
	...
	layer.addEventListener("click", (e) => {
		...
		else if (t?.closest('[data-act="sk-wipe"]')) ground.wipe(myUserId()) // my own water only
	})
	...
	function upsertRemote(d: RemoteGun) {
	...
	socket.on("gimmick-guns", (list: RemoteGun[]) => {
	...
	socket.on("gimmick-squirt", (d: Squirt) => {
		if (!d?.userId) return
		startBurst(d)
	})
