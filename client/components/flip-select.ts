...
/** One row of the menu; `style` is inline CSS for the row (how the typeface menu previews each face). */
...
export function flipSelectHtml(id: string, rows: FlipRow[] = [], value = "", { label = "" }: { label?: string } = {}): string {
	const cur: Partial<FlipRow> = rows.find((r) => r.value === value) || rows[0] || { label: "" }
	return (
		...
		`<span class="flip-cur" id="${esc(id)}Cur"${cur.style ? ` style="${esc(cur.style)}"` : ""}>${esc(cur.label ?? "")}</span><i class="chev">&#9662;</i></button>` +
...
export interface FlipSelectOpts {
	id?: string
	rows?: FlipRow[]
	value?: string
	label?: string
	onChange?: (value: string) => void
	/** lift the list to <body> while open, so it can escape a clipping parent */
	portal?: boolean
}
export interface FlipSelect {
	set(v: string): void
	readonly value: string
	close(): void
	el: HTMLElement
	/** exposed so a host (the theme switch) can treat clicks in it as its own */
	menu: HTMLElement
}
...
export function mountFlipSelect(root: HTMLElement, { id = "fs", rows = [], value = "", label = "", onChange, portal = false }: FlipSelectOpts = {}): FlipSelect {
	root.innerHTML = flipSelectHtml(id, rows, value, { label })
	const box = root.querySelector<HTMLElement>(".flip-select")!
	const toggle = box.querySelector<HTMLElement>(".flip-toggle")!
	const menu = box.querySelector<HTMLElement>(".flip-menu")!
	const cur = box.querySelector<HTMLElement>(".flip-cur")!
	...
	menu.addEventListener("click", (e) => {
		const b = (e.target as Element | null)?.closest<HTMLElement>("[data-val]")
		if (!b) return
