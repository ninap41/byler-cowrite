...
export function radiusAt(y: number, t: TowerSpec = TOWER): number {
...
export function bow(y: number, r: number, cx: number = TOWER.cx, dip: number = TOWER.curve): string {
...
function masonry(t: TowerSpec): string {
	const rows: string[] = []
	let i = 0
...
function shaft(t: TowerSpec): string {
	const rb = t.rBottom
	const rt = t.rTop
...
export function lancet(x: number, y: number, w: number, h: number, cls = "cl-glass"): string {
	const r = w / 2
	return `<path d="M ${x - r} ${y} v ${-h} a ${r} ${r} 0 0 1 ${w} 0 v ${h} Z" class="${cls}" />`
}

function openings(t: TowerSpec): string {
...
// ---- #cl-door ---------------------------------------------------------------
function door(t: TowerSpec): string {
...
function moss(t: TowerSpec): string {
	...
	const at = (y: number, f: number, rx: number, ry: number): [number, number, number, number] => {
		const r = radiusAt(y, t)
...
function balcony(t: TowerSpec): string {
	const y = t.shaftTopY
	const r = t.rTop + 24
	const corbels: string[] = []
	for (let i = -3; i <= 3; i++) {
...
function roof(t: TowerSpec): string {
...
// ---- #cl-dormer -------------------------------------------------------------
function dormer(t: TowerSpec): string {
...
function spire(t: TowerSpec): string {
	const y = t.roofTopY
...
export function towerSvg(t: TowerSpec = TOWER): string {
