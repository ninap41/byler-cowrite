...
import type { Gimmick, GimmickMountOpts, Table, Ack } from "./gimmick-types.js"
...
export function targetsHtml(writers: Table = [], myUserId: string | null = null): string {
...
export interface CurseOpts extends GimmickMountOpts {
	/** the table as game.html keeps it from game-state */
	getTable?(): Table
	/** rings the vecna clock under the sound prefs */
	playChime?(): void
}
export interface CurseGimmick extends Gimmick {
	readonly cursedUserId: string | null
	readonly typed: number
}
/** The curse on the wire: who it fell on (and for how long), or that it lifted. */
type CurseEvent = { targetUserId: string; targetName?: string; byName?: string; byColor?: string; duration?: number; lift?: false } | { targetUserId: string; lift: true }
interface Cursed {
	targetUserId: string
	targetName?: string
...
export function mountVecnaCurse(opts: CurseOpts): CurseGimmick {
	const { socket } = opts
	...
	let cursed: Cursed | null = null // whoever it's on, seen by all
	let typed = 0 // MY typed WORDS while I am the victim
	let inWord = false // a printable key starts a word; a space or Enter ends it
	let tick: ReturnType<typeof setInterval> | 0 = 0 // the countdown on the veil
	const paintWord = () => {
		if (!cursed || !mineNow()) return
		const left = Math.max(0, Math.ceil(((cursed.until ?? 0) - Date.now()) / 1000))
		word.textContent = `WRITE. ${typed}/${CURSE_LIFT_WORDS} words · ${left}s`
	}
	const mineNow = (): boolean => !!cursed && cursed.targetUserId === myUserId()

	function markChips(on: boolean) {
	...
	function fall(d: Exclude<CurseEvent, { lift: true }>) {
	...
	socket.on("gimmick-curse", (d: CurseEvent) => {
