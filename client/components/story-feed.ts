...
import type { StoryLine } from "../shared/wire.js"

export const EMPTY_STORY_HTML = '<p class="empty">The page is blank. The first line is coming…</p>'

export interface StoryHtmlOpts {
	/** lines at index >= freshFrom get the "fresh" entrance animation */
	freshFrom?: number
	/** the signed-in account id — that author's lines get ✎ edit and ✕ delete buttons */
	mineId?: string | null
}

export function storyHtml(story: StoryLine[], { freshFrom = Infinity, mineId = null }: StoryHtmlOpts = {}): string {
...
export function livePreviewHtml(html: string | null | undefined, name: string, color: string): string {
	const text = html || ""
