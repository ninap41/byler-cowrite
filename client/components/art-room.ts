...
/** A stroke as it rides the wire and the canvas: colour, width, screen-fraction points, eraser flag. */
...
export interface ArtRoomGimmick extends Gimmick {
	wipe(): void
	...
	strokesOf(userId: string): number
}
interface Painter {
	strokes: Stroke[]
	live: Stroke | null
...
const hexOr = (c: unknown, fallback: unknown): string => (typeof c === "string" && /^#[0-9a-f]{6}$/i.test(c) ? c.toLowerCase() : safeColor(fallback))
...
export const BRUSH_SIZES: readonly { id: string; size: number }[] = [
	{ id: "fine", size: 3 },
	{ id: "brush", size: 6 },
	{ id: "broad", size: 14 },
	{ id: "roller", size: 28 },
...
export function swatchRowHtml(myColor: unknown): string {
...
export function sizeRowHtml(selected: number = BRUSH_SIZE): string {
...
export const layerHtml = (): string => `<div class="ar-layer hidden" id="arLayer" aria-label="Art room">
...
export function mountArtRoom(opts: GimmickMountOpts): ArtRoomGimmick {
	const { socket } = opts
	const doc = opts.document || document
	const win = doc.defaultView || (typeof window !== "undefined" ? window : null)
	doc.body.insertAdjacentHTML("beforeend", layerHtml())
	const layer = doc.getElementById("arLayer")!
	const canvas = doc.getElementById("arCanvas") as HTMLCanvasElement
	...
	const fr = (v: number): number => clamp(v, 0, 1)
	...
	const paint = new Map<string, Painter>()
	...
	function sizeLayer(p: Painter) {
		if (!p.layer) return
		p.layer.width = canvas.width
		p.layer.height = canvas.height
		p.dirty = true
	}
	const layerOf = (p: Painter): HTMLCanvasElement | null => {
		if (!p.layer && ctx) {
	...
	function drawStroke(s: Stroke | null | undefined, target: CanvasRenderingContext2D | null | undefined = ctx) {
	...
	const painterOf = (userId: string | null): Painter => {
		userId ??= ""
	...
	// ---- my brush ----
	let open = false
	let curColor: string = safeColor(opts.getMyColor?.())
	let curErase = false // 🧽 selected: strokes take paint away
	let curSize = BRUSH_SIZE
	let stroke: Stroke | null = null // the stroke being painted right now
	const remote = new Map<string, HTMLElement>() // userId -> cursor el (their brush position)
	const syncLayer = () => {
	...
	function send(final = false) {
		const msg: { on: true; cursor?: [number, number]; stroke?: Stroke; live?: boolean } = { on: true }
	...
	function beginStroke(px: number, py: number) {
	...
	function extendStroke(px: number, py: number) {
		if (!stroke) return
	...
	const overChat = (x: number, y: number): boolean => {
	...
	layer.addEventListener("click", (e) => {
		...
		const sz = t?.closest<HTMLElement>(".ar-size")
		if (sz) {
		...
		const sw = t?.closest<HTMLElement>(".ar-swatch")
		if (sw) {
			curErase = sw.dataset.erase === "1"
			if (!curErase) {
				curColor = hexOr(sw.dataset.color, opts.getMyColor?.())
				const pick = colorsRow.querySelector<HTMLInputElement>("#arPick")
	...
	function exit({ fade = false, keepHint = false }: { fade?: boolean; keepHint?: boolean } = {}) {
		if (!open) return
		open = false
		painting = false
		if (stroke) endStroke()
	...
	function upsertCursor(d: StrokeEvent) {
	...
	function dropCursor(userId: string, { fade = false }: { fade?: boolean } = {}) {
	...
	socket.on("gimmick-stroke", (d: StrokeEvent) => {
	...
	socket.on("gimmick-paints", (list: (StrokeEvent & { strokes?: Stroke[]; live?: Stroke | boolean | null })[]) => {
		for (const d of list || []) {
