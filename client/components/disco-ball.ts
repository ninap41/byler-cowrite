...
export function ballHtml(key: string | number): string {
...
export const layerHtml = (): string => `<div class="db-layer hidden" id="dbLayer" aria-label="Disco ball">
	<div id="dbLights"></div>
	<div id="dbOthers"></div>
...
export function spotColors(ownerColor: string, tableColors: string[] = [], n = SPOTS_PER_SPIN): string[] {
...
export interface DiscoOpts extends GimmickMountOpts {
	getTableColors?(): string[]
}
export interface DiscoGimmick extends Gimmick {
	spin(): void
...
export function mountDiscoBall(opts: DiscoOpts): DiscoGimmick {
	const { socket } = opts
	...
	const setPos = (el: HTMLElement, px: number, py: number) => {
		...
		const chain = el.querySelector<HTMLElement>(".db-chain")
		if (chain) chain.style.height = Math.max(0, Math.round(py - BALL_W / 2)) + "px"
	}
	const ballInner = (key: string, name: string | undefined, color: string | undefined): string =>
	...
	function showCenter(ownerId: string): [number, number] {
	...
	function startShow(ownerId: string, ownerColor: string, duration: number) {
		...
		const spots: Spot[] = colors.map((color, i) => {
	...
	function endShow(ownerId: string | null, { fade = false }: { fade?: boolean } = {}) {
	...
	const remote = new Map<string, { el: HTMLElement; x: number; y: number }>()
	const syncLayer = () => {
	...
	function upsertRemote(d: RemoteBall) {
	...
	socket.on("gimmick-balls", (list: RemoteBall[]) => {
	...
	socket.on("gimmick-spin", (d: { userId: string; color: string; duration?: number }) => {
		if (!d?.userId) return
