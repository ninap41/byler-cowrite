...
export const CONFIRM_SOURCES = ["discord"] as const
export const needsJoinConfirm = (from: unknown): boolean => (CONFIRM_SOURCES as readonly string[]).includes(String(from || "").toLowerCase())

/** What /api/live says about a game (name, phase, a player count, the host). */
export interface JoinConfirmGame {
	code: string
	name?: string
	phase?: string
	players?: number
...
export function joinConfirmHtml(g: JoinConfirmGame): string {
