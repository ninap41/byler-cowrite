...
import type { HexColor } from "./brands.js"
...
export type Reaction = keyof typeof EMOJI_WORDS
export const REACTIONS = Object.keys(EMOJI_WORDS) as Reaction[]

/** One reactor on a chip: the key they react under (a userId for a seat, a socket id for a spectator). */
export interface Reactor {
	key: string
	name: string
	color?: HexColor
}
export type Reactions = Partial<Record<Reaction, Reactor[]>>

/** The part of a chat message the reactions care about (see docs/TYPES.md ChatMessage). */
...
export function searchReactions(q: unknown): Reaction[] {
...
export const cleanReaction = (e: unknown): Reaction | null => (typeof e === "string" && (REACTIONS as string[]).includes(e) ? (e as Reaction) : null)

const HEX = /^#[0-9a-f]{6}$/i
// a reactor's colour is only ever a validated hex (it lands in a style=)
const cleanTipColor = (c: unknown): HexColor | null => (HEX.test(String(c ?? "")) ? (String(c).toLowerCase() as HexColor) : null)
...
export function toggleReaction(msg: Reactable | null | undefined, key: string, name: unknown, emoji: unknown, color?: unknown): boolean {
...
export function reactionTipHtml(list: Reactor[] | undefined): string {
	const rows = (list || [])
		.filter((r) => r.name)
...
export function reactionsHtml(reactions: Reactions | null | undefined, myKey?: string | null): string {
	const out: string[] = []
...
export const reactionGridHtml = (reactions: Reactions | null | undefined, myKey?: string | null, q = ""): string => {
...
export const reactionPickerHtml = (reactions: Reactions | null | undefined, myKey?: string | null): string =>
