...
import { mountFlipSelect, type FlipSelect } from "./components/flip-select.js"
export { SITE_FONTS }
// The registry itself (THEMES, THEME_LABELS, DEFAULT_THEME) lives in
// ./shared/themes.js so the server can import it too; re-exported here so
// every existing consumer keeps its import.
import { THEMES, THEME_LABELS, DEFAULT_THEME, isThemeId, type ThemeId, type ThemeGate, type ThemeLock } from "./shared/themes.js"
export { THEMES, THEME_LABELS, DEFAULT_THEME }
export type { ThemeId, ThemeGate, ThemeLock }
...
export const cleanSiteFont = (key: unknown): string => (fontByKey(key) ? String(key) : DEFAULT_FONT)
export const themeAllowed = (id: string, { locks = {}, unlocked = [] }: Partial<ThemeGate> = {}): boolean => !locks[id] || unlocked.includes(id)
// "🧙 Sorcerer · 20,000 words" — what a locked row tells you.
export function lockTip(id: string, locks: Record<string, ThemeLock> = {}): string {
	const lock = locks[id]
	if (!lock) return ""
...
/** A GSAP tween, as far as this module drives one. */
interface TweenLike {
	kill?(): void
...
export function initTheme(): ThemeController {
	...
	let current: ThemeId = isThemeId(saved) ? saved : DEFAULT_THEME
	let floatTweens: TweenLike[] = []
	...
	let gate: ThemeGate = { locks: {}, unlocked: [] }
	...
	const REACH = 0.82
	function startParallax(theme: string) {
		if (stopScroll) stopScroll()
		stopScroll = null
		if (theme !== "cleradin" || reduce) return
		const layers = [...document.querySelectorAll<HTMLElement>(".bg-set.cleradin [data-par]")]
		...
		const rateOf = (l: HTMLElement): number => {
	...
	let cloudTweens: TweenLike[] = []
	function stopClouds() {
		cloudTweens.forEach((t) => t && t.kill && t.kill())
		cloudTweens = []
	}
	function startClouds(theme: string) {
		stopClouds()
		const bands = [...document.querySelectorAll<HTMLElement>(".bg-set.clouds .cloud-band")]
	...
	function startFloat(theme: string) {
		if (!gsap) return
		stopFloat()
		if (theme === "aurora") {
	...
	let menuOpen = false
	if (gsap && menu) menu.style.transition = "none" // CSS transition is the no-GSAP fallback only
	function openMenu() {
		menuOpen = true
	...
	if (menu)
		menu.addEventListener("click", (e) => {
			const b = (e.target as Element | null)?.closest<HTMLElement>("[data-theme-btn]")
			if (b) {
	...
	function setGate(next: Partial<ThemeGate> | null | undefined) {
	...
	const fontPick: FlipSelect | null = fontHost
		...
		: null
	function applyFont(key: unknown) {
