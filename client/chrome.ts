...
export interface KofiMount {
	open(): void
	close(): void
	modal: HTMLElement
	frame: HTMLIFrameElement
}
export function mountKofi(doc: Document = document): KofiMount {
	doc.body.insertAdjacentHTML("beforeend", KOFI_MODAL)
	const modal = doc.getElementById("kofiModal")!
	const frame = doc.getElementById("kofiFrame") as HTMLIFrameElement
...
export function setPeek(on: boolean): void {
...
let themeCtl: ThemeController | null = null
let themeLocks: Record<string, ThemeLock> = {}
...
export function mountChrome({ page = "", nav = true, kofi = true }: ChromeOpts = {}): ThemeController {
	...
	const h1 = document.querySelector<HTMLElement>(".wrap h1")
	...
	themeCtl = theme
	api<ThemeGate>("/api/themes", null, "GET")
		.then((d) => {
			themeLocks = d.locks || {}
			theme.setGate(d)
		})
	...
	// Every emoji on the page gets the <h1> gradient fill — except rank/badge
	// emojis and anything inside an editor or input (see EMOJI_SKIP). The
	// observer keeps re-rendered UI (chat, live games, toasts) gradiented too.
...
/** The slice of publicUser the top bar reads. */
...
export function setUserChip(user: ChipUser | null | undefined): void {
...
export function regateThemes(unlocked: unknown): void {
	if (Array.isArray(unlocked) && themeCtl) themeCtl.setGate({ locks: themeLocks, unlocked: unlocked as string[] })
}
